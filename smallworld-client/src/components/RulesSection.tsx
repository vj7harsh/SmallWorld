...
export function RulesSection({ embedded = false }: RulesSectionProps) {
  ...
  return (
    <div className={containerClass} style={embedded ? { color: '#2d3436' } : {}}>
      <div className="flex flex-col gap-3">
        {!embedded && <h2 className="text-2xl font-semibold">Rules Reference</h2>}

        <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
          <input
            className={embedded ? "w-full sm:flex-1 px-4 py-2 text-base outline-none" : "w-full sm:flex-1 rounded-xl border px-4 py-2"}
          ...
          <div className="flex flex-wrap gap-2">
            <button
              className={embedded ? "px-3 py-2 text-sm transition-all" : `rounded-xl border px-3 py-2 text-sm ${
                activeCategory === "All" ? "font-semibold" : "opacity-80"
              }`}
              style={embedded ? (activeCategory === "All" ? buttonActiveStyle : buttonBaseStyle) : {}}
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                style={embedded ? (activeCategory === cat ? buttonActiveStyle : buttonBaseStyle) : {}}
          ...
          return (
            <div key={cat} className="flex flex-col gap-3">
              <h3
                ...
                style={embedded ? { color: '#2d3436', textShadow: '1px 1px 0px rgba(0,0,0,0.1)' } : {}}
              >
                {cat}
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {items.map((item) => (
                  <details
                    key={item.id}
                    className={embedded ? "p-4" : "rounded-2xl border p-4 shadow-sm"}
                      ...
                      {item.tags && item.tags.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-2">
                          {item.tags.map((t) => (
                            <span
                              key={t}
                              className={embedded ? "text-xs rounded-full px-2 py-1" : "text-xs rounded-full border px-2 py-1 opacity-80"}
